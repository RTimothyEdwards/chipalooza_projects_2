 ...
 *
 * 5) connect GPIO 27 to ground (reference low)
 * 6) connect GPIO 28 to power (reference high)
 * 7) connect GPIO 35 to ground (trim)
 * 8) connect GPIO 30 to a mid-range voltage
 *	(preferably vdda/2 or 1.65V, but 1.8V
 *	should work for most purposes.)
 * 9) connect GPIO 31 to an input source, to be used in
 *	ADC mode.
 *
 ...
 *-----------------------------------------------------------
 *
 * 1) enable power supply for CDAC
 * 2) power supply is monitored on GPIO 21.
 * 3) present sawtooth wave on the digital input, incrementing
 *    with the blink cycles.
 * 4) view analog output on GPIO 29, although this may be
 *    meaningless to measure without buffering.
 *
 * Measurements:  Measure linearity, monotonicity, rail-to-rail
 * operation, INL, and DNL.
 *
 * Might be a good idea to connect the the sample and hold
 * at the output, as this will provide output buffering.
 * Need to understand the characteristics of the sample &
 * hold and incorporate them into the CDAC measurements.
 */
...
void main()
{
    ...
    // Enable the power switch to the CDAC
    cdac_enable();
    value = 0;
    ...
    // When should CDAC "hold" be applied?
