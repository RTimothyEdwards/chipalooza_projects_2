 ...
 *
 *-----------------------------------------------------------
 * 1st test:  Sample at the blink interval.  Check
 * response.  Output should be level with a slow drift.
 * Measure the drift and the delta between the input and
 * output voltages.
 *-----------------------------------------------------------
 *
 * 1) enable power supply for sample & hold
 * 2) present analog input on GPIO 12
 * 2) view analog output on GPIO 13
 *
 * Measurements:  Measure drop of the output signal
 * over time.  Measure the difference between the input
 * and the sampled output.
 *
 * Note that the sampling may be affected by the amount
 * of time that the "hold" signal stays low.  Adjust the
 * delay on this pulse accordingly (see code below).
 */
...
void main()
{
    ...
    sample_and_hold_enable();

    // If the LED on the board is blinking,
    // GPIO 13 should be the output and should be periodically
    // sampling the value at GPIO 12.
