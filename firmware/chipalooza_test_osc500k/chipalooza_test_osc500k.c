 ...
 *
 *-----------------------------------------------------------
 * 1st test:  Check basic output
 *-----------------------------------------------------------
 *
 * 1) enable power supply for oscillator
 * 2) connect the oscillator output
 * 3) power supply monitor is GPIO 18 (no ESD protection)
 * 7) view digital output on GPIO 13.
 *
 * Basic functional test:  Output should be running at
 * 500kHz at STP.
 *
 * Measurements:  Measure the change in frequency vs.
 * power supply and temperature.  Measure the startup
 * and shutdown times.
 *
 * Use osc500k_set_enable(1|0) to turn local enable on
 * and off.
 */
...
void config_io() {
    ...
    reg_mprj_io_13 = GPIO_MODE_USER_STD_OUTPUT;		/* digital out */
