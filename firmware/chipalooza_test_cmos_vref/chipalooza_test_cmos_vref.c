 ...
 *
 *-----------------------------------------------------------
 * 1st test:  Check output levels
 *-----------------------------------------------------------
 *
 * 1) enable power supply for LDO (note this is a 1.8V circuit)
 * 2) power supply monitor is GPIO 23 (no ESD protection)
 * 3) view analog outputs:
 *	vbg on GPIO 26, should be 1.2V nominal.
 *	vbgsc on GPIO 29
 *	vbgtg on GPIO 30
 *	vptat on GPIO 25
 *
 * Basic functional test:  Vbg output should be 1.2V at STP.
 *
 * Measurements:  Measure the power supply rejection and
 * load response.  Test the output value vs. trim setting
 * using cmos_vref_set_trim().
 *
 * See experiment suggested in the LDO test directory,
 * configuring the LDO to provide the 1.8V output to
 * drive the digital vccd2 power supply.
 */
...
void main()
{
    ...
    // Enable the power switch to the voltage reference.
    cmos_vref_enable();
